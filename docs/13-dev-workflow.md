...
> v1.4 (2026-09-11) — §3.2에 **통합 테스트 뒷정리 규약**(회사를 지우기 전에 `audit_log`를 먼저 지운다) 신설(#301)
...
`@Transactional` 없이 도는 통합 테스트는 `@AfterEach`에서 직접 지운다. 그때
**회사를 지우기 전에 감사 로그를 먼저 지운다.**
...
`audit_log.company_id`가 `company(id)`를 참조하므로(V1 356행) 행이 남아 있으면
**회사 삭제가 외래키로 막힌다.** 감사 로그는 리스너가 자동으로 남기므로 테스트가
직접 넣지 않아도 생긴다 — `AuditLogListener`가 받는 일곱(`QuoteSent`·`DealStageChanged`·
`QuoteViewed`·`QuoteApproved`·`QuoteRejected`·`OrderCreated`·`MemberDeactivated`) 중 하나라도
유발하면 그렇다. 단계 이동·발송·열람·승인·반려·주문 전환·구성원 비활성화가 그 동작이다.

**지금 통과하는 것이 안전하다는 뜻은 아니다.** 회사를 지우는 통합 테스트 34건 중
감사 로그를 먼저 지우는 것은 9건이고, 나머지 25건은 위 동작을 **하지 않아서** 통과한다.
그 테스트에 동작 한 줄이 늘어나는 순간 깨진다 — 새 테이블이 늘 때만이 아니라
**기존 테스트가 한 줄 자랄 때**도 걸리는 자리다.

공통 헬퍼로 모으는 것은 **v1 밖이다**(#301). 전부 초록인 테스트 25개를 마감 앞에
건드리는 것은 위험 대비 이득이 없다 — 뒷정리를 잘못 옮기면 판별력이 조용히 사라진다.
그때까지는 이 규약 한 줄이 대신한다.
