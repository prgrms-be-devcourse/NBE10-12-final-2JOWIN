...
```java
// 수신 빈 — 커밋 후에 받는다. 롤백된 작업이 기록으로 남으면 안 된다
...
// 쓰기 빈 — 반드시 다른 Bean 이어야 한다 (v2.0.21)
@Async @Transactional(REQUIRES_NEW) void writeAsync(AuditLog row)
```

**`@ApplicationModuleListener`를 쓰지 않는다** — 그것은 `@Async` + `@Transactional(REQUIRES_NEW)` +
`@TransactionalEventListener(AFTER_COMMIT)`를 한 메서드에 합친 것이다. 그러면 비동기 제출이 프록시에서
일어나 **거부 예외를 그 메서드 안 `try/catch`로 잡을 수 없다** (`AsyncConfig` javadoc). 수신과 쓰기를
나누고, 제출이 거부되면 수신 빈이 그 자리에서 직접 적재한다 — 감사 로그에는 메일의 `email_log = SCHEDULED`
같은 선행 내구 기록이 없어 거부를 흘리면 그 사건이 어디에도 남지 않는다.
...
| B | `AuditQuery` (**stageChanges** — 기간 안의 단계 전이 이력; DB-07 전환율의 원천. `payload` 대신 `before`·`after`를 꺼내 주고 집계는 하지 않는다 · 행위자 축 없음 — 자동 전이가 `SYSTEM`이라 actor로 담당자별 집계를 하면 그 건들이 빠진다) (v2.0.21) | C |
