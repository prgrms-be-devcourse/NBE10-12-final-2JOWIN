...
| C | `QuoteQuery` (응답 대기·임박 후보 — NT-05·06, DB-03) (v2.0.1) · **originsByIds·quoteIdsByDeals** (주문의 견적 경유 조회·SC-04 범위, OD-08·09) (v2.0.10) · **QuoteSummary에 dealId·companyId** (호출자가 거르는 축) (v2.0.11) · **briefsByDeals** — 딜 상세의 견적 요약(회수·반려 포함, 그 딜의 이력이다); `QuoteBrief`에 `dealId`를 실어 호출자가 묶는다 (v2.0.23, #304) | D, C(order) |
