...
public interface DealQuery {
    ...
    long countOpenAssigned(UUID companyId, UUID memberId);   // A의 MB-14 판정 — 진행 중만 센다(종결 제외). 0이면 이관 대상 생략 (v2.0.7)
}

public interface DealCommand {                   // C 구현 — 다른 도메인의 사건으로 Deal이 움직이는 시스템 전이만 (v2.0.7)
...
| C | `DealCommand` (promoteToQuoteStage — 발송 시 자동 승급 Q-25 · reassignOpenDeals — 비활성화 시 진행 중 담당 Deal 이관, 옮긴 id 반환 MB-14) (v2.0.7) | C(quote), A |
