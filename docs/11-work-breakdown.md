...
**버전:** v2.0 (2026-08-26) · **v2.0.29** (2026-09-11 — §3 리스너 코드 블록에서 `@Transactional(REQUIRES_NEW)`를 리스너 메서드에서 떼어 별도 빈 `AuditLogWriter.save()`로 옮긴다(#328). 리스너 메서드에 직접 붙이면 프록시가 메서드 바깥에서 트랜잭션을 시작·커밋하므로, 시작 실패(커넥션 풀 고갈)와 커밋 시 flush 실패(id 가 `GenerationType.UUID`라 INSERT 는 커밋 때 나간다)가 리스너의 `try`를 비껴 `AFTER_COMMIT` 경로로 새고 이미 커밋된 요청이 500 이 된다. `REQUIRES_NEW`가 필요한 이유는 그대로다 — 자리만 `try` 안으로 들어간다. 프록시가 하는 일은 메서드 안의 `try`로 못 잡는다는 `AsyncConfig` javadoc 의 원리와 같다) · **v2.0.28** (2026-09-11 — §7.2 `QuoteCommand`에 `expireOnDealLost(dealId)` 추가(#318). 전이표 §5 실패 처리 행의 **효과**("진행 중 견적 → 기간 만료, 열람 링크 → 만료(DEAL_LOST)")가 구현되어 있지 않았다 — `deal.stage`만 LOST가 되고 견적은 SENT·VIEWED로, 링크는 ACTIVE로 남아 **고객이 실패 처리한 딜의 견적을 승인할 수 있었다**(승인은 견적 상태만 본다). 사유 enum에 `DEAL_LOST`가 있는데 그 값을 넘기는 호출자가 없던 자리다. **`MANDATORY`다** — 실패 처리와 한 트랜잭션이어야 하고, 딜만 LOST이고 견적 일부가 열린 상태는 전이표에 없다. 기간 만료 배치(Q-37)가 건별 `REQUIRES_NEW`인 것과 갈리는 이유는 대량 처리가 아니어서다 — 담당자 요청 한 건이라 부분 성공이 의미가 없다. 닫는 대상 판정은 `Quote.expire()`가 하고 만료 배치와 **같은 메서드**를 써서 규칙이 두 벌이 되지 않게 한다. 같은 PR에서 "진행 중"(발송됨·열람됨)의 정의를 `Quote.IN_PROGRESS` 한 곳으로 모은다 — 응답 대기 조회·만료 배치·이 계약 셋이 따로 적고 있었다 (2026-09-11 C 확정)) · **v2.0.27** (2026-09-11 — §7.2 `SalesStatsQuery` 행의 `conversions`를 실구현으로 정정(#307). 마지막 자리표시자였다. 정의는 **도달 기준**이다 — 도달(S)=그 딜이 S 이상 단계에 도달한 적 있음, rate(X→Y)=|도달(Y) 고유 딜| / |도달(X) 고유 딜|, 쌍은 인접 4개 고정(D 확정). 진입 이벤트가 아니라 도달로 세면 되돌리기(DL-08)와 왕복이 모델 안에서 접히고, 자동 승급(Q-25)·자동 성사(OD-06)도 최고 도달에 반영돼 역전이 쌍이 결과에 나오지 않는다. **모집단은 기간 안에 등록된 딜이다** — 전이 이력이 아니다. `audit_log`에는 움직인 딜만 남아 리드에 멈춘 딜이 분모에서 통째로 빠지고(딜 생성은 감사 이벤트가 아니다), 그러면 전환율이 늘 1 근처로 나온다 — 전환율이 드러내야 할 바로 그 딜들이다. 그래서 도달을 세 원천의 최댓값으로 잡는다: `deal.stage`(정체 딜까지 포함하는 바닥) · `deal.lost_from_stage`(실패 딜의 도달 지점, LOST는 순서 밖) · `audit_log`의 `afterStage`(되돌린 딜의 봉우리 — 여기서만 얻을 수 있다). **이력이 `deal`을 대체하지 않고 보정한다**는 뜻이라, 리스너(v2.0.21, #287) 이전 기간을 물어도 수치가 무너지지 않는다 — 되돌린 딜의 봉우리만 놓친다. 기간을 무엇에 거는지는 D 확정 넷에 없어 등록일 코호트로 정했다(#322 리뷰 항목). 딜이 없는 기간도 빈 목록이 아니라 rate 0인 네 행이다 — 화면의 네 칸이 흔들리지 않게 한다. **시그니처에 `today`가 붙는다** — `from`·`to`는 등록일 범위이고 도달 판정에는 기간 제한이 없어 코호트 딜의 `to` 이후 전이도 봐야 한다. 계약이 시간을 스스로 읽지 않는 규약대로 호출자가 넘긴다 — `DashboardController`가 기본 기간을 만들 때 이미 가진 값이다 (#322 리뷰) (2026-09-11 C·D·E 확정)) · **v2.0.26** (2026-09-11 — §7.2 `ViewTokenQuery`에 `recipientContactIdOf`(견적 id → 활성 열람 토큰의 수신 연락처) 추가(#249, NT-06). NT-06 임박 메일이 견적을 받은 고객사 담당자에게 가야 하는데 그 연락처는 D의 `quote_view_token`에만 있어 C가 `QuoteSummary`에 못 채운다. §5 핵심 규칙 표에 NT-06 행 확정 — 메일 전용·인앱 가드 없음·`email_log` UNIQUE + 예약 전 사전 체크가 견적당 1회. `findExpiringBetween` 실구현(#315)을 `ExpiringQuoteIntegrationTest`(실 PG)로 확인해 cron을 활성화한다) · **v2.0.25** (2026-09-11 — §5 `ViewTokenCommand.expire`의 인자를 `String reason` → 중첩 enum `ExpiredReason`으로 정정. 코드는 2026-08-27(#13)부터 enum인데 표기만 문자열로 남아 있었다. 값도 3종만 적혀 있어 **TIME·RESENT가 빠졌다** — 만료 배치(Q-37)와 재발송(AP-13)이 어느 사유로 링크를 닫는지 문서만 봐서는 알 수 없었다. **같은 절 안에서도 어긋나 있었다** — 소유 테이블 표는 `expired_reason 5종(TIME/MANUAL/WITHDRAWN/RESENT/DEAL_LOST)`이라 적고 바로 아래 `issue()` 주석은 "기존 활성 링크 EXPIRED(RESENT)"라 적는데 `expire()` 주석만 3종이었다. 계약상 C 몫은 넷(TIME·MANUAL·WITHDRAWN·DEAL_LOST)이고 `RESENT`는 `issue()` 안에서 D가 붙인다는 것도 함께 적는다 — 구현된 호출자는 셋(회수 QT-17·수동 만료 AP-14·만료 배치 #316)이고 `DEAL_LOST`는 호출자가 없다(#318) — AP-13·14의 구성원용 엔드포인트가 C 소유라서 생기는 구분이다(11 §4). 값 집합은 `quote_view_token.expired_reason`의 CHECK와 1:1이라 바꾸면 마이그레이션이 함께 가야 한다. 같은 PR에서 README 인덱스의 06 행을 실제 버전(v1.6.7)에 맞추고, 머지 충돌 해소에서 유실된 v2.0.22(#227)·v2.0.23→v2.0.24(#297) 변경 이력을 복원한다) · **v2.0.24** (2026-09-11 — §5·§7.2 `QuoteQuery.findExpiringUntil(date)`를 `findExpiringBetween(from, to)`로 확정(NT-06). 상한만 받으면 이미 만료된 견적이 후보에 섞인다 — 견적 만료 배치(Q-37)가 아직 없어 `valid_until`이 지난 SENT 견적이 남아 있고, 그 건에 "유효기간 임박" 메일이 나가면 틀린 안내다. 하한을 계약 안에서 "오늘"로 만들지 않는 것은 규약이다 — 계약은 시간을 스스로 정하지 않고 호출자가 넘긴다(`requireSendable(today)`·`OrderQuery.wonTotalsByQuotes`와 같은 축). 배치가 하루 건너뛰어도 `from`을 늘려 따라잡는다. **`companyId`는 받지 않는다** — 전 회사를 한 번에 돌려주고 호출자가 `QuoteSummary.companyId`로 그룹핑해 회사당 `CompanyQuery.get` 1회로 Q-27을 판정한다(v2.0.11이 그 필드를 넣은 목적). `findAwaitingResponse`가 회사별인 것과 다른 것은 각 배치가 자기 쿼리 모양에 맞춘 결과다. 이름을 바꾼 이유는 `Until`이 상한만 받는 것처럼 읽혀서다 — 소비처가 D의 NT-06 배치뿐이라 지금이 바꿀 시점이다. 구현은 스텁 유지, 실구현은 후속 (2026-09-10 C·D 확정 · 실구현은 #315). **원래 v2.0.23으로 #297에 실렸으나 머지 충돌 해소에서 헤더 항목과 §7.2 행이 유실됐고, 그 번호는 #304가 썼다** — §5 시그니처 줄만 남아 있던 것을 여기서 복원한다) · **v2.0.23** (2026-09-11 — §7.2에 `QuoteQuery.briefsByDeals`·`OrderQuery.briefsByQuotes` 신설(#304). 딜 상세(DL-15)의 견적·주문 탭이 `List.of()` 하드코딩이라 승인 견적과 주문이 달린 딜에서도 0으로 떴다 — 09-10 게이트 리허설에서 커트라인 10번의 대체 경로가 죽어 있는 것으로 드러난 자리다. 기존 `quoteIdsByDeals`는 주문 목록의 범위 필터라 id만 주는데, 화면은 견적번호·상태·금액을 그려야 해서 같은 조회를 두 번 지나게 된다. `wonAmount`(DL-18)는 새 창구 없이 이 주문 줄의 합으로 낸다 — `wonTotalsByQuotes`는 기간 집계(DB-02·06)의 자리이고, 같은 행을 두 번 읽을 이유가 없다. 성사 전에는 `null`이다(0을 넣으면 화면이 "주문 0원"으로 읽는다). `briefsByQuotes`는 `wonTotalsByQuotes`와 달리 **null을 "제한 없음"으로 받지 않는다** — 딜 상세는 언제나 특정 딜로 좁혀진 자리라, 빈 묶음에 회사 전체 주문이 붙으면 그대로 사고다) · **v2.0.22** (2026-09-10 — §7.2 `DealCommand` 세 메서드를 `MANDATORY`로 확정(#227). 계약 javadoc이 "호출자의 트랜잭션에 합류한다"로 약속하는데 기본 `REQUIRED`는 호출자 트랜잭션이 없으면 조용히 자기 것을 열어, "발송이 롤백되면 단계도 되돌아간다"가 그 경로에서 흔적 없이 깨졌다. PR #197에서 정한 기준(호출자 트랜잭션 필수면 MANDATORY)을 그대로 적용한 것이고, 현재 호출부 셋은 전부 쓰기 트랜잭션 안이다 — `QuoteService.send` · `OrderService.convert` · `MemberAdminService.deactivate`. **`MANDATORY`는 `readOnly`를 걸러내지 못한다**는 한계는 javadoc에 명시했다(PR #225 리뷰) — 읽기 전용 호출자는 통과하고 flush가 건너뛰어져 변경이 사라진다) · **v2.0.21** (2026-09-10 — §7.2 인터페이스 표에 `AuditQuery` 행 신설(#287). `stageChanges`(기간 안의 단계 전이 이력)는 DB-07 전환율의 원천이다. 기간은 한국 날짜로 받는다 — `occurred_at`이 이 모듈 컬럼이라 날짜를 시각으로 끊는 규칙도 여기 둔다(#216 리뷰가 `OrderQuery`에서 정한 것과 같은 축) — `audit_log`가 B 소유라 경계 없이는 읽을 수 없고, `payload`를 그대로 내보내지 않고 `before`·`after`를 꺼내 준다(저장 형식이 경계를 넘으면 규약이 바뀔 때 소비자가 함께 깨진다). 집계는 하지 않는다 — 전환율의 정의와 분모가 소비자 쪽에 있어 여기서 세면 같은 규칙이 두 곳에 생긴다. · §3 리스너 형태를 실제 구조로 정정 — `@ApplicationModuleListener`로 적혀 있었으나 그것은 `@Async`를 포함한 형태다. **적재는 동기로 한다** — 비동기로 넘기면 제출 거부와 큐 유실이라는 실패 지점이 둘 더 생기는데 감사 로그에는 그 유실을 되짚을 선행 기록이 없고, 얻는 것은 응답 몇 밀리초뿐이라 메일과 사정이 다르다. `REQUIRES_NEW`는 필요하다 — `AFTER_COMMIT` 시점에는 원래 트랜잭션이 이미 커밋돼 있어 합류하면 `INSERT`가 커밋되지 않는다) · **v2.0.20** (2026-09-10 — §7.2 `DealQuery.DealSummary`에 `customerId` 추가(#273). B의 `namesByIds`(v2.0.17)를 소비해도 N+1이 절반만 풀렸다 — 주문은 경계를 거쳐야 해서 딜→고객사를 줄마다 `customerIdOf`로 되짚고 있었고, 그건 B의 창구 문제가 아니라 이 계약의 구멍이었다. `summariesByIds` 한 번으로 딜과 고객사 id를 함께 얻어 그 묶음을 `namesByIds`에 한 번 넘긴다. 이름이 아니라 <b>id</b>를 싣는 이유는 고객사가 B 소유라 이 계약이 이름을 실을 수 없어서다. 같은 PR에서 응답 대기(DB-03)의 `customerName` 공백도 해소된다 — 견적에는 고객사 id가 없어 deal을 한 홉 더 지난다) · **v2.0.19** (2026-09-10 — §7.2에 `OrderQuery` 행 신설(#216). 성사 금액은 예상 금액이 아니라 주문 합계라(DL-18) `orders`를 읽어야 하는데 집계는 deal 모듈이라 order 쪽 창구가 필요했다. 범위는 판정하지 않는다 — 축(`deal.assignee_member_id`)을 가진 호출자가 견적 id로 좁혀 넘기고, `quoteIds`가 null이면 회사 전체·빈 목록이면 아무것도다(`OrderSpecs`와 같은 규약). 기간은 한국 날짜로 받는다 — `created_at`이 order 소유 컬럼이라 경계 계산도 이 모듈이 갖는다. 견적당 주문은 최대 하나라(`orders.quote_id` UNIQUE, OD-03) 견적당 한 행이고 전환 건수는 행 수다. 같은 PR에서 `SalesStatsQuery` 행을 "pipeline만 실구현" → "conversions만 자리표시자"로 고친다) · **v2.0.18** (2026-09-10 — §7.2 `QuoteCommand` 행에 `ConversionSnapshot.Line`의 `sortOrder` 반영(#208). `order_item`에 순서 컬럼이 없어 주문 항목이 조회마다 다른 순서로 나왔다 — `quote_item`은 `sort_order`를 갖고 있었는데 전환 스냅샷이 그 축을 옮기지 않았다. 컬럼만 추가하고 스냅샷을 그대로 두면 신규 주문이 전부 0이 되므로 계약을 함께 넓힌다. 기존 행은 `created_at, id` 순으로 0-base 백필했다 — 원래 순서를 복원할 축이 없으니 "조회마다 갈리지 않게 고정"이 도달 가능한 상한이다) · **v2.0.17** (2026-09-10 — §2·§7.2 `CustomerQuery`에 `namesByIds`(고객사 id 묶음 → 이름 배치 조회) 추가(#218). `QuoteSummary.customerName`이 `null` 고정이던 원인이 이 창구의 부재였다 — 단건 `get`은 `AccessContext`를 요구하는데 목록 조립 지점에는 그 값이 없고, 줄마다 부르면 N+1이 된다. 소비처는 DB-03 응답 대기 카드와 주문 목록이다. 규약은 `DealQuery.summariesByIds`와 같다 — 순서 보장 없음 · 없는 id는 예외 없이 빠짐 · 소프트 삭제 제외 · 빈 목록이면 빈 목록) · **v2.0.16** (2026-09-09 — §1.3에 PATCH 규약 포인터 한 줄 신설(#233). `null`=미변경 · nullable은 `''`로 비움이 08 §B(v1.6.7)에만 있어, 화면·목을 맞출 때마다 08의 개별 record 주석을 뒤져야 했다(#204에서도 근거를 다시 찾았다). §1은 「공통 개발 컨벤션」의 정본이므로 여기에 **포인터만** 둔다 — 규범 문장을 복제하면 정본이 둘이 되어 갈라진다. 주문 착수일·납기(OD-10)가 이 규약의 예외라는 것도 함께 적는다 — 두 날짜를 함께 덮어써 `null`이 지움이고, 실 API 전환(#248)에서 목이 이것을 미변경으로 받고 있던 것을 찾았다) · **v2.0.15** (2026-09-09 — §5 NT-05·06 배치 규칙에 인앱 레그 서술 보강(#231). 무응답 리마인드 배치(`RemindNoResponseBatch`)는 메일 중복을 `email_log` UNIQUE로, 인앱 중복을 `notification`(REMIND_NO_RESPONSE·해당 견적) 존재 가드로 막아 견적당 1회를 보장한다 — 담당자 재배정 시 재알림하지 않는다) · **v2.0.14** (2026-09-09 — §7.2 인터페이스 표에 `CompanyQuery` 행 신설(#238). 표에 아예 빠져 있던 계약이다. `findActiveIds`(활성 회사 id 전체)를 함께 추가한다 — NT-05·06 배치가 회사 단위로 순회하는 진입점으로, 배치에는 `AccessContext`가 없어 회사 스코프를 인자로 받을 수 없다(v2.0.11 `QuoteSummary`와 같은 자리). 정지 회사는 빠지고(Q-27), 목록은 호출 시점의 스냅샷이라 회사별 처리 직전에 `get`으로 다시 본다) · **v2.0.13** (2026-09-09 — §7.2 `ViewTokenQuery`에 `quoteIdOf`(발송 토큰 id → 견적 id) 추가(#212). NT-12 최종 실패 알림이 `email_log.ref_id`(발송 토큰 id)에서 담당 구성원을 되짚는 통로 — 없으면 `Optional.empty()`) · **v2.0.12** (2026-09-09 — §7.1 `ViewTokenCommand.issue`에 `message`(담당자 한마디, 08 `SendQuoteRequest.message`) 추가 — 발송 모달의 메시지가 안내 메일에 실리도록(#183)) · **v2.0.11** (2026-09-08 — `QuoteQuery.QuoteSummary`에 `dealId`·`companyId` 추가(#205). 배치에는 `AccessContext`가 없어 SC-02를 계약 안에서 판정할 수 없다 — 회사 전체를 돌려주고 호출자가 이 두 축으로 거른다. `findAwaitingResponse`·`SalesStatsQuery.pipeline` 실구현 반영) · **v2.0.10** (2026-09-08 — §7.2 인터페이스 표에 `DealCommand`(promoteToQuoteStage·markWon) 행 추가(#109·#160, 표에 빠져 있었다) · `QuoteCommand`에 `lockApprovedForConversion` · `QuoteQuery`에 `originsByIds`·`quoteIdsByDeals` 반영(#160)) · **v2.0.9** (2026-09-08 — §7.2 인터페이스 표에 `ActivityQuery`(recent — DB-04 최근 활동)·`TaskQuery`(followUps — DB-05 후속 필요) 행 추가(#158·#178) · 계약 javadoc의 summary 규칙·OWNED_ONLY 판정 정정) · **v2.0.8** (2026-09-08 — §7.2 인터페이스 표에 `DealCommand`(promoteToQuoteStage·reassignOpenDeals) 행 추가(#109·#130) · `DealQuery` 행에 customerIdOf·summariesByIds·assignedDealIds·countOpenAssigned 반영(#32·#46·#109·#130)) · **v2.0.7** (2026-09-08 — §7.2 인터페이스 표에 `PublicQuoteAssembler`(assembleForPreview·assembleForView — 고객 화면 조립 통로; QT-12·AP-02) 행 추가(#163)) · **v2.0.6** (2026-09-07 — §7.2 인터페이스 표에 `NotificationSettingQuery`·`NotificationSettingCommand`(settingsOf·replaceSettings) 행 추가(#127)) · **v2.0.5** (2026-09-04 — §7.2 인터페이스 표에 `NotificationCommand`(notify·notifyForDeal) 행 추가(#75)) · **v2.0.4** (2026-09-03 — §7.2 인터페이스 표에 `MailCommand`(schedule) 행 추가(#47) · §2 `MemberQuery`에 A 내부 인터페이스 3건 추가(#40)) · **v2.0.3** (2026-08-27 — 프론트엔드 소유 확정: E가 오너, 플랫폼 + 로그인·고객 열람 담당 / 도메인 화면은 각 백엔드 담당자 · 게이트에 프론트 조건 추가 · Seed=목 픽스처. 상세는 `12-frontend-plan.md`) · **v2.0.2** (화면 설계 공백 반영 — `QuoteCommand.approve/reject`에 응답자 정보(AP-19) · `QuoteQuery` 반환에 첫 열람 시각(GAP-08)) · **검수 보정 v2.0.1** (2026-08-26 — 인터페이스 보강(§4·§5·§7.2: CU-08·12·14와 D 대시보드·배치의 데이터 통로 신설) · NT-12 수신자 규칙 참조 · WON 전이 단계 무관 정합 · AU-09 잠금 판정 문구 정정)
...
```java
// 커밋 후에 받아 그 자리에서 적재한다. 롤백된 작업이 기록으로 남으면 안 된다
// 리스너 메서드에는 트랜잭션이 없다 — 저장은 별도 빈 AuditLogWriter.save()가 REQUIRES_NEW 로 하고,
// 리스너는 그 호출을 try 로 감싼다 (v2.0.29, #328)
@TransactionalEventListener(AFTER_COMMIT) void on(DealStageChanged e)   // C 발행 (advance·revert·lose·reopen·자동 승급·자동 성사)
@TransactionalEventListener(AFTER_COMMIT) void on(QuoteSent e)          // C 발행
@TransactionalEventListener(AFTER_COMMIT) void on(QuoteViewed e)        // C 발행 (D가 quote.markViewed() 호출 → C 도메인이 발행)
@TransactionalEventListener(AFTER_COMMIT) void on(QuoteApproved e)      // C 발행 (경계 합의 — 상태 변경 주체가 C이므로 발행도 C)
@TransactionalEventListener(AFTER_COMMIT) void on(QuoteRejected e)      // C 발행
@TransactionalEventListener(AFTER_COMMIT) void on(OrderCreated e)       // C 발행
@TransactionalEventListener(AFTER_COMMIT) void on(MemberDeactivated e)  // A 발행
```

**비동기로 넘기지 않는다** (v2.0.21). 스레드 풀에 제출하면 실패 지점이 둘 더 생긴다 — 제출 거부와
큐에 든 채 서버 사망이다. 감사 로그에는 메일의 `email_log = SCHEDULED` 같은 선행 내구 기록이 없어
그 유실을 되짚을 수단이 없다. 얻는 것은 응답 몇 밀리초인데 행 하나 `INSERT`라 체감되지 않는다.
메일이 비동기인 것은 SMTP 를 기다리기 때문이고 여기는 다르다.

`REQUIRES_NEW`는 필요하다 — `AFTER_COMMIT` 시점에는 원래 트랜잭션의 동기화 정리가 아직 끝나지
않았을 수 있어, 합류하려 들면 커밋되지 않거나 예외가 난다 (`AsyncConfig` javadoc).
**다만 리스너 메서드가 아니라 별도 빈에 붙인다** (v2.0.29) — 리스너 메서드에 직접 붙이면 트랜잭션
시작·커밋이 메서드 바깥에서 일어나 그 실패가 `try`를 비껴 커밋된 요청을 500 으로 뒤집는다.
...
public interface SalesStatsQuery {               // D 소비 — 대시보드 집계 (DB-01~08, v2.0.1 보강. SC 범위는 ctx로 적용)
    ...
    List<StageConversion> conversions(UUID companyId, LocalDate from, LocalDate to,
                                      LocalDate today);   // DB-07 — from·to는 등록일, today는 도달 상한 (v2.0.27)
}
```
...
public interface ViewTokenCommand {      // C가 호출 (경계 합의의 역방향)
    void issue(UUID quoteId, UUID recipientContactId, String message);   // 발송·재발송 시 발급 (기존 활성 링크 EXPIRED(RESENT)) · message = 담당자 한마디, null·공백이면 생략 (v2.0.12, #183)
    void expire(UUID quoteId, ExpiredReason reason);   // 중첩 enum 5종 — 계약상 C 몫은 TIME · MANUAL · WITHDRAWN · DEAL_LOST (RESENT는 issue()가 붙인다) · 구현된 호출자는 셋, DEAL_LOST는 #318 (v2.0.25)
}

public interface ViewTokenQuery {        // B가 CU-14, notification이 NT-12·NT-06에서 호출 (v2.0.13·18)
    boolean existsForContact(UUID contactId);          // 발송 이력(수신인 지정 이력) 존재 — true면 CONTACT_HAS_QUOTES로 삭제 차단
    Optional<UUID> quoteIdOf(UUID tokenId);            // 발송 토큰 id → 견적 id (NT-12 실패 알림 수신자 되짚기, 없으면 empty)
    Optional<UUID> recipientContactIdOf(UUID quoteId); // 견적 id → 활성 열람 토큰의 수신 연락처 (NT-06 임박 메일 수신자, 없으면 empty) (v2.0.26)
}
```
...
| C | `QuoteCommand` (markViewed·approve·reject · **lockApprovedForConversion** — 주문 전환용 행 잠금 + 스냅샷, OD-01·02·04) (v2.0.10) · **`ConversionSnapshot.Line`에 `sortOrder`** — 주문이 견적 항목 순서를 물려받는 축 (v2.0.18) · **expireOnDealLost**(`MANDATORY`) — 딜 실패 시 진행 중 견적·링크를 같은 트랜잭션에서 닫는다 (DL-10, 전이표 §5) (v2.0.28) | D, C(order) |
...
| C | `QuoteQuery` (응답 대기·임박 후보 — NT-05·06, DB-03) (v2.0.1) · **originsByIds·quoteIdsByDeals** (주문의 견적 경유 조회·SC-04 범위, OD-08·09) (v2.0.10) · **QuoteSummary에 dealId·companyId** (호출자가 거르는 축) (v2.0.11) · **briefsByDeals** — 딜 상세의 견적 요약(회수·반려 포함, 그 딜의 이력이다); `QuoteBrief`에 `dealId`를 실어 호출자가 묶는다 (v2.0.23, #304) · **findExpiringUntil → `findExpiringBetween(from, to)`** — 전 회사·양 끝 포함, 대상은 SENT·VIEWED (v2.0.24, NT-06 · #297 복원) | D, C(order) |
| C | `SalesStatsQuery` (대시보드 집계 — DB-01~08) (v2.0.1) · **넷 다 실구현** — pipeline·monthlyWon·performance (v2.0.19, #216) · `conversions`는 도달 기준·등록일 코호트, 인접 4쌍 고정·rate 0~1 (v2.0.27, #307). 되돌린 딜의 봉우리만 `audit_log`의 `STAGE_MOVED`에 기댄다 | D |
| C | **`OrderQuery`** (wonTotalsByQuotes — 기간 안에 전환된 주문, DB-02·06) (v2.0.19, #216) · **briefsByQuotes** — 딜 상세의 주문 요약; 빈 묶음이면 빈 목록(null=제한없음을 받지 않는다) (v2.0.23, #304) | C(deal) |
| D | `ViewTokenCommand` (issue(+message v2.0.12)·expire) | C |
| D | `ViewTokenQuery` (existsForContact — CU-14 · **quoteIdOf** — NT-12 실패 알림 토큰→견적 되짚기 (v2.0.13) · **recipientContactIdOf** — NT-06 임박 메일 수신 연락처, 견적 id→활성 토큰 연락처 (v2.0.26)) | B, D |
...
| D | `PublicQuoteAssembler` (assembleForPreview·assembleForView — 고객 화면 조립 통로; QT-12·AP-02) (v2.0.7) | C, D |
| D | `NotificationSettingQuery` (settingsOf — NT-07 메일 수신 설정 조회, 4종 채워 반환) | A |
| D | `NotificationSettingCommand` (replaceSettings — NT-07 전체 교체 upsert) | A |
