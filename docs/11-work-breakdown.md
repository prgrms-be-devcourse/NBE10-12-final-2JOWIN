...
| B | `ProductQuery` | C |
| C | `QuoteCommand` (markViewed·approve·reject · **lockApprovedForConversion** — 주문 전환용 행 잠금 + 스냅샷, OD-01·02·04) (v2.0.7) | D, C(order) |
| C | `DealCommand` (promoteToQuoteStage·markWon — **시스템 전이** 전용, Q-25·OD-06) (v2.0.7) | C(quote·order) |
| C | `DealQuery` (assigneeIdOf·isOpen·hasOpenDeals·summariesByCustomer) | B, D |
| C | `QuoteQuery` (응답 대기·임박 후보 — NT-05·06, DB-03) (v2.0.1) · **originsByIds·quoteIdsByDeals** (주문의 견적 경유 조회·SC-04 범위, OD-08·09) (v2.0.7) | D, C(order) |
