...
| 버전 | 변경 | 근거 |
| --- | --- | --- |
| v1.6.7 | **audit_log payload 봉투 명시(2026-09-09)** — 변경 필드를 `changes`로 감싸고 부가 필드는 최상위에 둔다. 두 종류가 최상위에 섞이면 "값이 바뀐 것"과 "표시용으로 딸려온 값"을 구별할 수 없어, 감사 상세의 `changes`에 무엇을 담을지가 값 모양 추측에 걸린다. 08이 "B가 이벤트 포맷 정의 시 준수"로 위임한 그 정의(이슈 #22 §2)가 이 형태인데 문서에는 담기지 않았다. 적재 리스너가 아직 없어 실데이터가 쌓이기 전인 지금이 맞추는 시점이다. 스키마 변경 없음 — 컬럼은 그대로 `jsonb`다 | #244 · AC-11 |
...
| payload | jsonb |  | **비밀번호·토큰·해시 저장 금지** — 인증 이벤트 포함으로 더 중요해짐 · 견적·주문 이벤트에는 **dealId 포함**(AC-06 Deal 타임라인 병합 키, v1.6.1) · **변경된 필드는 `changes` 안에, 표시용 부가 필드는 최상위에** — `{"dealId": …, "changes": {"stage": {"before": …, "after": …}}}`. 발생형 이벤트는 `changes` 키가 없다 (v1.6.7) |
