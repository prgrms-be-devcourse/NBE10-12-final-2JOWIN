...
> **v1 구현 한계** (v1.6.16) — **단계 전환율만** `SalesStatsQuery` 자리표시자라 빈 목록이다(0% 아님) — `audit_log`의 `STAGE_MOVED` 적재가 선행이다. 이달 성사·담당자별 실적은 실집계다(#216) — 다만 화면 노출은 프론트 플래그 해제(#283)가 따로 필요하다. 응답 대기 목록은 영업 담당자도 본인 담당 Deal로 실필터한다(#207).
