...
| 버전 | 변경 |
| --- | --- |
| v1.6.15 | **§D 대시보드 파라미터 기본값·v1 구현 한계 명시(2026-09-08)** — `month` 미지정 시 서버 시간(KST) 당월, `from`/`to` 미지정 시 이달 1일~오늘. 형식 오류·`from > to`·366일 초과는 400 `VALIDATION_FAILED`. **영업 담당자의 응답 대기 목록은 v1에서 빈 배열** — `QuoteQuery.findAwaitingResponse`가 `AccessContext` 없이 `companyId`만 받고 `QuoteSummary`에 담당 축이 없어 본인 필터가 불가능한데, 회사 전체 견적 노출은 누수라 닫는다 (C가 `QuoteSummary.dealId` 추가 시 실필터, #207). **이달 성사·담당자별 실적·단계 전환율은 "집계 준비 중"** — `SalesStatsQuery`의 해당 셋이 자리표시자라 빈 값·0으로 나가므로 화면은 "0"이 아니라 준비 중으로 표시한다(주문 합계는 orders 경계 창구, 전환율은 전이 이력 테이블 — 각각 별도 이슈). 발견 경로: 대시보드 API 구현(#202) |
...
> **파라미터 기본값** — `month` 미지정 시 서버 시간(KST) 당월, `from`/`to` 미지정 시 이달 1일~오늘. 형식 오류·`from > to`·366일 초과는 400 `VALIDATION_FAILED`.
> **v1 구현 한계** (v1.6.15) — ① 영업 담당자의 응답 대기 목록은 빈 배열(누수 방지 — `QuoteSummary.dealId` 추가 시 실필터, #207) ② 이달 성사·담당자별 실적·단계 전환율은 `SalesStatsQuery` 자리표시자라 "집계 준비 중"으로 표시(0 아님).
