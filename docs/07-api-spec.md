...
| 버전 | 변경 |
| --- | --- |
| v1.6.20 | **§C 재발송 경로 정합 — 안내 문구 + 담당자 한마디(2026-09-11, #300·#214)** — `QUOTE_VALID_UNTIL_PASSED` 문구가 "유효기간을 다시 지정한 뒤 발송해 주세요"였는데 **재발송 경로에서는 거짓**이다: 발송된 견적의 `validUntil`은 `Quote.update`가 `requireDraft`로 막는다(QT-16). 04 §5는 고객에게 "담당자에게 재발송을 요청해 주세요"라 안내하는데, 그 요청을 받은 담당자에게 남는 행동이 없었다. **복제(QT-19)가 #310으로 들어와 행동이 생겼다** — 두 경로에서 다 참인 안내로 바꾼다: **"유효기간이 지났습니다. 견적을 복제해 새로 보내 주세요."** `QUOTE_NOT_RESENDABLE`에도 같은 행동을 붙인다 — 기간 만료 배치(Q-37, #316)가 돌면 이쪽이, 돌기 전이면 저쪽이 나오므로 **두 코드가 같은 상황을 다르게 말하면 안 된다**. 경로별 화면 안내는 두지 않는다 — 문구가 두 경로에서 참이면 갈라놓을 이유가 없다. · **`ResendViewTokenRequest`에 `message` 추가(#214)** — 첫 발송에만 담당자 한마디가 실려 **새 수신인이 첫 수신인보다 빈약한 메일**을 받고 있었다. 수신인을 바꾸는 흐름이라 앞선 맥락이 없는 사람이 받는 자리다. **선택 필드**이고 비우면 종전과 같다. 응답·상태 코드 변경 없음 |
...
| **QUOTE_NOT_RESENDABLE** | 409 | **발송됨·열람됨이 아닌 견적의 수신인 변경 재발송 (`/view-token/resend`, AP-13).** 전이표 §7의 재발송 전이는 출발이 활성 링크뿐이다. 판정 축은 링크가 아니라 **견적 상태** — 수동 만료(AP-14) 뒤 다른 수신인에게 다시 보내는 정상 흐름을 막지 않기 위해서다 (v1.6.7). **안내는 복제(QT-19)다** — `QUOTE_VALID_UNTIL_PASSED`와 같은 행동이어야 한다: 기간 만료 배치(Q-37)가 돌면 이쪽이, 돌기 전이면 저쪽이 나오는 **같은 상황**이다 (v1.6.20) |
| **QUOTE_VALID_UNTIL_PASSED** | 409 | **유효기간이 지난 견적의 발송·재발송 (`/send` · `/view-token/resend`).** 입력은 `@Future`로 막지만(08) 저장된 값이 시간이 지나며 낡는다 — 만료 배치(Q-37)는 발송됨·열람됨만 대상이라 오래된 작성 중 견적은 그대로 남는다. 재검증이 없으면 **열자마자 만료된 링크**가 나간다 (v1.6.7). **안내는 복제다** — "유효기간을 다시 지정"은 발송(`/send`, DRAFT라 수정 가능)에서만 참이고 재발송에서는 거짓이었다 (v1.6.20) |
...
| **QUOTE_NOT_RESENDABLE** | **409** | **이 상태의 견적은 재발송할 수 없습니다. 복제해 새 견적으로 보내 주세요.** |
| **QUOTE_VALID_UNTIL_PASSED** | **409** | **유효기간이 지났습니다. 견적을 복제해 새로 보내 주세요.** |
