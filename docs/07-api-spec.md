...
| 버전 | 변경 |
| --- | --- |
| v1.6.20 | **§C 재발송 경로 정합 — 부록 문구는 중립, 행동 안내는 화면이 경로별로(2026-09-11, #300·#214)** — `QUOTE_VALID_UNTIL_PASSED` 문구가 "유효기간을 다시 지정한 뒤 발송해 주세요"였는데 **재발송에서는 거짓**이다: 발송된 견적의 `validUntil`은 `Quote.update`가 `requireDraft`로 막는다(QT-16). 04 §5 180행은 고객에게 "담당자에게 재발송을 요청해 주세요"라 안내하는데, 그 요청을 받은 담당자에게 남는 행동이 없었다. **그렇다고 "복제하세요"로 바꾸면 이번엔 발송 경로가 차선이 된다** — 작성 중이면 편집기에서 날짜만 고치면 되는데 복제하면 DRAFT가 하나 더 남는다(#342 리뷰). `QUOTE_NOT_RESENDABLE`은 더 갈린다: **DRAFT·승인됨에서도 나므로** 복제가 맞는 것은 반려·회수·만료뿐이다. 그래서 **부록은 상태만 알린다** — `"유효기간이 지났습니다."` · `"이 상태의 견적은 재발송할 수 없습니다."` 행동 안내는 **화면이 경로별로** 한다(`QuoteActionDialogs`의 `ACTION_HINTS`): 발송 모달은 "유효기간을 다시 지정", 재발송 모달은 "복제해 새 견적으로". 부록에 한쪽 행동을 박으면 다른 쪽에서 틀린 안내가 되기 때문이다. · **`ResendViewTokenRequest`에 `message` 추가(#214)** — 첫 발송에만 담당자 한마디가 실려 **새 수신인이 첫 수신인보다 빈약한 메일**을 받고 있었다. 수신인을 바꾸는 흐름이라 앞선 맥락이 없는 사람이 받는 자리다. **선택 필드**이고 비우면 종전과 같다. 응답·상태 코드 변경 없음 |
| v1.6.19 | **§C `supersededByQuoteId`(QT-28) 규칙 확정 — 항상 `null`이던 값을 채운다(2026-09-11, #326)** — 전용 컬럼 없이 `cloned_from_quote_id`의 **역방향**으로 구한다(06 ERD 368행이 이미 두 용도로 규정). 규칙: **원본이 반려·회수됐고, 그 복제본 중 `DRAFT`가 아닌 것 가운데 `sent_at`이 가장 최근인 것.** 없으면 `null`. 셋이 한 판단이다 — **"대체"는 고객에게 다시 간 것만 뜻한다**(QT-28 "반려·회수된 견적에서 그것을 **대체한** 새 견적"). ①진행 중·기간 만료 원본은 제외한다(대체될 이유가 없고, 만료는 시스템 전이라 "다시 제안"이 아니다) ②`DRAFT` 복제본은 제외한다(끝내 발송되지 않으면 화면의 "대체한 견적으로 이동"이 거짓이 되고 되돌릴 수 없다) ③여럿이면 `created_at`이 아니라 **`sent_at`** 최신이다(먼저 만든 복제본을 나중에 보낼 수 있다). 복제본이 다시 반려·회수되면 사슬이 이어지고, 중간이 `SENT`·`EXPIRED`로 남으면 거기서 끊긴다. **상세 응답에만 싣는다** — 목록에는 넣지 않는다. 선행: 견적 복제(QT-19, #310) |
| v1.6.18 | **§B 감사 로그 기간 파라미터 타입 명시(2026-09-11)** — `GET /api/v1/audit-logs`의 `from`·`to`가 표에 이름만 있어 서버는 `Instant`, 화면은 날짜 선택기(`yyyy-MM-dd`)로 서로 다르게 짐작했고, 프론트가 KST 경계를 계산해 시각으로 바꿔 보내는 것으로 메워져 있었다(#129). **`yyyy-MM-dd` 한국 날짜**로 확정한다 — `to`는 그날을 포함한다. 같은 성격의 대시보드 기간(v1.6.15)·주문 목록과 같은 형태이고, 날짜를 시각으로 끊는 규칙은 `occurred_at`을 소유한 활동 모듈이 갖는다(11 §7.2 `AuditQuery`와 같은 축). 응답·에러 변경 없음. 발견 경로: 프론트 실 API 전환(#129) · #289 |
| v1.6.17 | **§D 단계 전환율(DB-07) 실구현 — 자리표시자 목록이 비었다(2026-09-11, #307)** — 정의는 **도달 기준**이다: 도달(S)=그 딜이 S 이상 단계에 도달한 적 있음, rate(X→Y)=|도달(Y) 고유 딜| / |도달(X) 고유 딜|, 쌍은 **인접 4개 고정**(`LEAD→CONSULT`·`CONSULT→QUOTE`·`QUOTE→NEGOTIATION`·`NEGOTIATION→WON`), `rate`는 0~1 소수로 **셋째 자리 반올림**. **모집단은 `from`~`to`에 등록된 딜**(코호트)이고 전이 이력이 아니다 — 이력에는 움직인 딜만 남아 리드에 멈춘 딜이 분모에서 빠지면 수치가 늘 1 근처가 된다. **도달 판정에는 기간 제한이 없다** — 코호트 딜의 전이는 `to` 이후에도 이어지므로 조회 시점까지 본다(#322 리뷰). 딜이 없는 기간도 빈 목록이 아니라 `rate` 0인 **네 행**이다. 되돌린 딜(DL-08)의 봉우리만 `audit_log`의 `STAGE_MOVED`에 기대므로, 리스너(#287) 이전에 되돌려진 딜은 그만큼 낮게 나온다 — **화면 노출**("집계 준비 중" 해제)은 별개 판단이다. 응답 형태는 바뀌지 않았다 |
| v1.6.16 | **§D 대시보드 v1 구현 한계 축소(2026-09-10)** — 이달 성사(DB-02)·담당자별 실적(DB-06·08)이 실집계가 되어 자리표시자 목록에서 뺀다(#216). 남는 것은 단계 전환율(DB-07) 하나이고, 그것은 `audit_log`의 `STAGE_MOVED` 적재가 선행이라 빈 목록이다. **백엔드가 실값을 내는 것과 화면에 뜨는 것은 다르다** — 프론트의 `SALES_STATS_PENDING` 해제는 #283이다. 응답 형태는 바뀌지 않았다 |
| v1.6.15 | **§D 대시보드 파라미터 기본값·v1 구현 한계 명시(2026-09-08)** — `month` 미지정 시 서버 시간(KST) 당월, `from`/`to` 미지정 시 이달 1일~오늘. 형식 오류·`from > to`·`from`~`to` 간격 366일 이상은 400 `VALIDATION_FAILED`. **영업 담당자의 응답 대기 목록은 본인 담당 Deal로 실필터** — `QuoteQuery.QuoteSummary.dealId`(#207) + `DealQuery.assignedDealIds`로 본인 담당 견적만 남긴다. **이달 성사·담당자별 실적·단계 전환율은 "집계 준비 중"** — `SalesStatsQuery`의 해당 셋이 자리표시자라 빈 값·0으로 나가므로 화면은 "0"이 아니라 준비 중으로 표시한다(주문 합계는 orders 경계 창구, 전환율은 전이 이력 — 실구현 #216). 발견 경로: 대시보드 API 구현(#202) |
...
| GET · PUT | /api/v1/me/notification-settings | 알림 수신 설정 (메일 채널만, Q-23) — **설정 대상 4종 · PUT은 전체 교체 (각주 4)** | 전 구성원 | NT-07 |
...
>
> (각주 3) **비밀번호 변경에는 시도 제한을 두지 않는다 (v1).** `login_attempt` 재사용은 방어를 공격 도구로 바꾼다 — AU-09 잠금은 email 기준 판정(`ix_login_attempt_email_time`)이라, 탈취된 세션의 공격자가 일부러 틀려 **정당 사용자의 로그인을 `LOGIN_LOCKED`로 잠글 수 있다.** 미도입 근거: refresh 쿠키가 `HttpOnly`·`Path` 한정이라 토큰을 외부로 빼낸 공격자의 대입 창은 access 수명 15분으로 제한되고(Q-32), 변경에 성공하면 전 세션이 폐기돼(전이표 §9) 정당 사용자가 즉시 알아챈다 — 조용한 탈취가 성립하지 않는다. **재검토 조건 — 지속 XSS는 `/api/v1/auth/refresh`를 반복 호출해 이 창을 무한히 연장할 수 있다. 2차 Redis 도입 시 memberId 기준 별도 카운터 + 전용 에러 코드로 함께 넣는다** (`LOGIN_LOCKED` 재사용 불가 — 429 문구가 변경 화면에 맞지 않는다. `CURRENT_PASSWORD_MISMATCH`를 401에서 분리한 것과 같은 이유).

> (각주 4) **NT-07 설정 대상은 4종이고 PUT은 전체 교체다 (v1.6.12).** 대상은 03 §2.13 채널 확정표에서 "인앱 + 메일(설정 가능)"인 알림뿐이다 — `QUOTE_VIEWED`(NT-03) · `QUOTE_RESPONDED`(NT-04 승인·반려 한 토글) · `REMIND_NO_RESPONSE`(NT-05) · `INQUIRY_RECEIVED`(NT-10). **`EMAIL_FAILED`(NT-12)는 인앱 전용이라 대상이 아니고**(Q-35), NT-14(재설정 안내)는 끄면 계정 복구가 막혀 대상이 아니다. **GET은 저장 여부와 무관하게 항상 4종을 다 돌려준다** — 저장된 행이 없는 항목은 `true`(기본 ON, 06 `notification_setting` 주석 · 11 §5). **PUT은 4종을 전부 담아야 한다** — 일부만 받으면 "안 보낸 항목을 어떻게 할지"에서 갈려 구성원이 예전에 끈 설정이 되살아난다. 네 항목이 정확히 한 번씩 오지 않거나 위 넷이 아닌 `type`이 오면 400 `VALIDATION_FAILED`다. **응답 순서는 위 나열 순서로 고정한다** — 순서가 흔들리면 화면의 토글이 자리를 바꾼다.
...
| GET | /api/v1/audit-logs?entityType=&from=&to= | 변경 감사 **목록** (요약 — payload 제외) — `from`·`to`는 **`yyyy-MM-dd` 한국 날짜**, `to`는 그날 포함 (v1.6.18) | 기업 관리자 | AC-11 |
...
| GET | /api/v1/quotes/{id} | 상세 (항목·추적·**supersededByQuoteId** — 반려·회수된 견적이면 마지막으로 **발송된** 복제본, 없으면 `null`; v1.6.19) 🔶 | 전 구성원 | AP-06·07, QT-28 |
...
| POST | /api/v1/quotes/{id}/view-token/resend | 수신인 변경 재발송 (contactId · **message?** — 담당자 한마디, v1.6.20) 🔶 | 전 구성원 | AP-13 |
...
| **QUOTE_NOT_RESENDABLE** | 409 | **발송됨·열람됨이 아닌 견적의 수신인 변경 재발송 (`/view-token/resend`, AP-13).** 전이표 §7의 재발송 전이는 출발이 활성 링크뿐이다. 판정 축은 링크가 아니라 **견적 상태** — 수동 만료(AP-14) 뒤 다른 수신인에게 다시 보내는 정상 흐름을 막지 않기 위해서다 (v1.6.7). **문구는 상태만 알린다** — DRAFT·승인됨에서도 나므로 복제가 맞는 것은 반려·회수·만료뿐이다. 행동 안내는 화면이 경로별로 한다 (v1.6.20) |
| **QUOTE_VALID_UNTIL_PASSED** | 409 | **유효기간이 지난 견적의 발송·재발송 (`/send` · `/view-token/resend`).** 입력은 `@Future`로 막지만(08) 저장된 값이 시간이 지나며 낡는다 — 만료 배치(Q-37)는 발송됨·열람됨만 대상이라 오래된 작성 중 견적은 그대로 남는다. 재검증이 없으면 **열자마자 만료된 링크**가 나간다 (v1.6.7). **문구는 상태만 알린다** — "유효기간을 다시 지정"은 발송(`/send`, DRAFT라 수정 가능)에서만 참이고 재발송에서는 거짓이다. 반대로 "복제하세요"는 발송에서 차선이라, 행동 안내는 화면이 경로별로 한다 (v1.6.20) |
...
| **COMPANY_SUSPENDED** | 409 | **정지된 회사의 견적 승인·반려·문의 — 열람(GET)은 허용 (SC-10, Q-27)** |
| **QUOTE_NOT_RESPONDABLE** | **409** | **열람됨이 아닌 견적에 승인·반려 — 링크는 유효한데 견적 상태가 어긋난 경우 (전이표 §6)** |
...
> **파라미터 기본값** — `month` 미지정 시 서버 시간(KST) 당월, `from`/`to` 미지정 시 이달 1일~오늘. 형식 오류·`from > to`·`from`~`to` 간격 366일 이상은 400 `VALIDATION_FAILED`.
> **v1 구현 한계** (v1.6.17) — **`SalesStatsQuery`에 자리표시자가 없다**. 단계 전환율(DB-07)도 실집계다(#307) — 도달 기준·등록일 코호트·인접 4쌍·`rate` 0~1 셋째 자리 반올림(위 변경 이력). 다만 되돌린 딜의 봉우리는 `audit_log` 적재(#287) 이후 것만 잡히므로 초기 수치는 실제보다 낮다. 이달 성사·담당자별 실적도 실집계다(#216) — 화면 노출은 프론트 플래그 해제(#283)가 따로 필요하다. 응답 대기 목록은 영업 담당자도 본인 담당 Deal로 실필터한다(#207).
...
| **QUOTE_NOT_RESPONDABLE** | **409** | **C 견적 (고객 응답, #126)** |
...
| **QUOTE_VALID_UNTIL_PASSED** | **409** | **유효기간이 지났습니다.** |
| QUOTE_DEAL_CLOSED | 409 | 종결된 Deal에는 견적을 작성할 수 없습니다. 새 Deal을 만들어 진행해 주세요. |
| **QUOTE_NOT_RESPONDABLE** | **409** | **이 상태의 견적에는 응답할 수 없습니다.** |
