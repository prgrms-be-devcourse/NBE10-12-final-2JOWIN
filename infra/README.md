...
| 4 | **메인 스택 apply** | **CI** | develop 에 머지 → `infra-ci-cd.yml` 이 승인 게이트를 거쳐 apply |
...
앞의 둘이 없으면 `terraform init` 이 `The attribute "bucket" is required` 로, 뒤의 둘이 없으면 `plan` 이 `No value for required variable` 로 죽는다. 그래서 `infra-ci-cd.yml` 의 게이트가 값이 없으면 잡을 아예 건너뛴다 — 첫 apply 전까지 모든 인프라 PR 이 빨간불이 되는 것을 막는다.
...
| `.github/workflows/infra-ci-cd.yml` | terraform fmt · validate · tflint · checkov · Infracost · plan / apply |
